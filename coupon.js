...
Cart.prototype.applyCoupons = function() {
	...
	this.items.forEach(function(elem,idx) {
        if (elem instanceof Coupon) {
        	if (elem.type == "P") { 
        		self.items.forEach(function (cartitem, cartidx) {
        		...
        		for (var i=idx;i<self.items.length;i++) {
...
var items = [new Coupon("N",".05"),new CartItem("9 inch Pan",19.99),
             new Coupon("A",4.0,"Planter",2), new CartItem("Planter",10.00), 
             new Coupon("P","0.10"), new Coupon("Planter",10.00)]
