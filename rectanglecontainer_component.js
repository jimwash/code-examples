define(['jquery', 'underscore', 'backbone', 'rect_component'], function($, _, Backbone, Rect) {
	...
	*/
	...
	RectContainer.prototype.drawRects = function(svg, startx, starty) {
		...
		if (this.rect) {
			...
			svg.append("rect").style('fill', 'none').style('stroke', 'black').style('stroke-width', '1px').attr("x", startx + "px").attr("y", starty + "px").attr("width", this.rect.width + "px").attr("height", this.rect.height + "px");
